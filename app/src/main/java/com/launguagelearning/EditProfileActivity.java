...
public class EditProfileActivity extends BaseActivity {
    ...
    private void submitdata(String fname,String lname,String uname,String pwd) {
        ...
        Call<ResponseData> call = apiService.update_profile(fname,lname,uname,pwd);
    ...
    private void getProfileData(String uname) {
        pd = new ProgressDialog(EditProfileActivity.this);
        pd.setTitle("Please wait,Data is being loaded...");
        pd.show();
        EndPointUrl apiService = RetrofitInstance.getRetrofitInstance().create(EndPointUrl.class);
        Call<List<EditProfile>> call = apiService.getProfile(uname);
        call.enqueue(new Callback<List<EditProfile>>() {
            ...
            public void onResponse(Call<List<EditProfile>> call, Response<List<EditProfile>> response) {
                pd.dismiss();
                editFirstName.setText(response.body().get(0).getFname());
                editLastName.setText(response.body().get(0).getLname());
                editPassword.setText(response.body().get(0).getPwd());
