...
public class AddTestQuestionsAdminActivity extends BaseActivity implements EasyPermissions.PermissionCallbacks {
    ProgressDialog pd;
    Button btn_submit;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_add_test_questions_admin);
        getSupportActionBar().setHomeButtonEnabled(true);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        getSupportActionBar().setTitle("Add Model Test");

        if (ContextCompat.checkSelfPermission(AddTestQuestionsAdminActivity.this, Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(AddTestQuestionsAdminActivity.this,new String[]{
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        EasyPermissions.onRequestPermissionsResult(requestCode, permissions, grantResults, AddTestQuestionsAdminActivity.this);
    }
    @Override
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_GALLERY_CODE && resultCode == Activity.RESULT_OK) {
            ...
            if (EasyPermissions.hasPermissions(this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                String filePath = getRealPathFromURIPath(uri, AddTestQuestionsAdminActivity.this);
    ...
    private void uploadImageToServer(){
        pd=new ProgressDialog(AddTestQuestionsAdminActivity.this);
        pd.setTitle("Loading");
        pd.show();
        ...
        Call<ResponseData> fileUpload = uploadImage.add_model_test(fileToUpload, map);
        fileUpload.enqueue(new Callback<ResponseData>() {
            @Override
            public void onResponse(Call<ResponseData> call, Response<ResponseData> response) {
                pd.dismiss();
                Toast.makeText(AddTestQuestionsAdminActivity.this, response.body().message, Toast.LENGTH_LONG).show();
               // finish();
            }
            @Override
            public void onFailure(Call<ResponseData> call, Throwable t) {
                pd.dismiss();
                Toast.makeText(AddTestQuestionsAdminActivity.this, "Error"+t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
