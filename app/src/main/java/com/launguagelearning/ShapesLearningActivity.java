...
public class ShapesLearningActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void getLessions() {
        pd = new ProgressDialog(ShapesLearningActivity.this);
        pd.setTitle("Please wait,Data is being loaded...");
        pd.show();
        EndPointUrl apiService = RetrofitInstance.getRetrofitInstance().create(EndPointUrl.class);
        Call<List<Lessons>> call = apiService.getShapesLessions();
                ...
                if (response.code()==200) {
                    if (response.body() != null) {
                        if(response.body().size()>0) {
                            ...
                            mViewPagerAdapter = new AnimalViewPagerAdapter(ShapesLearningActivity.this, response.body());
